...
# Reuse existing nodebuilder-wg-easy container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-wg-easy; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-wg-easy -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=ghcr.io/wg-easy/wg-easy:v15.0.0" \
    "${container}"
# Commit the image
